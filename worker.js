export default {
  async fetch(request, env) {
    ...
    if (url.pathname === "/api/submit" && request.method === "POST") {
      ...
      const required = ["fullName", "phone", "projectTitle", "description", "pricingType"];
      for (const field of required) {
        if (!data[field] || String(data[field]).trim() === "") {
          return json({ ok: false, error: `فیلد ${field} الزامی است` }, 400);
        }
      }
      const allowedPricing = ["negotiable", "custom_amount", "awaiting_quote"];
      if (!allowedPricing.includes(data.pricingType)) {
        return json({ ok: false, error: "نوع قیمت‌گذاری نامعتبر است" }, 400);
      }
      if (data.pricingType === "custom_amount" && !data.pricingAmount) {
        return json({ ok: false, error: "رقم پیشنهادی الزامی است" }, 400);
      }

      try {
        await env.DB.prepare(
          `INSERT INTO project_submissions
            (full_name, phone, email, project_title, service_type, readiness, description, quantity, deadline, pricing_type, pricing_amount, notes)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
        ).bind(
          ...
          data.pricingType, data.pricingAmount ? Number(data.pricingAmount) : null,
          data.notes || null
        ).run();
        return json({ ok: true });
      } catch (e) {
    ...
    if (url.pathname === "/api/admin/list" && request.method === "POST") {
      ...
      const { results } = await env.DB.prepare(
        `SELECT id, full_name, phone, email, project_title, service_type, readiness,
                description, quantity, deadline, pricing_type, pricing_amount, notes, status, created_at
...
function render(rows){
  ...
  let html = '<table><tr><th>#</th><th>نام</th><th>موبایل</th><th>پروژه</th><th>توضیحات</th><th>قیمت‌گذاری</th><th>وضعیت</th><th>تاریخ</th></tr>';
  for(const r of rows){
    const pricingLabel = { negotiable: 'توافقی', custom_amount: 'رقم پیشنهادی: ' + (r.pricing_amount||'-') + ' تومان', awaiting_quote: 'منتظر اعلام قیمت' }[r.pricing_type] || r.pricing_type || '-';
    html += '<tr><td>'+r.id+'</td><td>'+esc(r.full_name)+'</td><td>'+esc(r.phone)+'</td><td>'+esc(r.project_title)+'</td><td>'+esc(r.description)+'</td><td>'+esc(pricingLabel)+'</td><td>'+esc(r.status)+'</td><td>'+esc(r.created_at)+'</td></tr>';
  }
  html += '</table>';
